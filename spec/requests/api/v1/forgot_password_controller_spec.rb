  ...
  context 'forgotPassword' do
    ...
    context 'negative' do
      it 'should not send an email to reset password for invalid email' do
        credentials = {user: {email: "invalid email"}}
  ...
  context "Reset Password" do
    context "Positive" do
      it "should reset the password with a valid password_reset_token" do
        params = {user: {password: "Password@1", password_confirmation: "Password@1", password_reset_token: user.password_reset_token}}
    ...
    context "Negative" do
      it "should not update the password if the password_reset_token expires" do
        ...
        params = {user: {password: "Password@1", password_confirmation: "Password@1", password_reset_token: user1.password_reset_token}}
        put "/api/v1/reset_password", params
        expect(response.status).to eq(422)
        body = JSON.parse(response.body)
